...
table_exists=$(psql -U $DB_USER -h localhost -d $DB_NAME -t -c "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'schema_migrations')")

if [[ $table_exists == *"f"* ]]; then
  echo "Creating schema_migrations table..."
  psql -U $DB_USER -h localhost -d $DB_NAME -c "CREATE TABLE schema_migrations (
...
for file in migrations/*.sql; do
  ...
  if ! psql -U $DB_USER -h localhost -d $DB_NAME -t -c "SELECT 1 FROM schema_migrations WHERE version = '$version'" | grep -q 1; then
    echo "Applying migration: $file"
    psql -U $DB_USER -h localhost -d $DB_NAME -f "$file"
    psql -U $DB_USER -h localhost -d $DB_NAME -c "INSERT INTO schema_migrations (version) VALUES ('$version')"
  fi
done
